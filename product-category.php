    ...
    if( ($_REQUEST['type'] != 'top-category') && ($_REQUEST['type'] != 'mid-category') && ($_REQUEST['type'] != 'end-category') ) {
        ...
        if($_REQUEST['type'] == 'top-category') {
            if(!in_array($_REQUEST['id'],$top)) {
                ...
                $arr1 = array();
                $arr2 = array();
                // Find out all ecat ids under this for allowed mid categories only
                        ...
                        if($prod_count==0) {
                            ...
                            for($ii=0;$ii<count($final_ecat_ids);$ii++) {
                                ...
                                foreach ($result as $row) {
                                    ...
                                    $statement_size = $pdo->prepare("SELECT t2.size_name FROM tbl_product_size t1 JOIN tbl_size t2 ON t1.size_id=t2.size_id WHERE t1.p_id=?");
                                    $statement_size->execute(array($p_id));
                                    $result_size = $statement_size->fetchAll(PDO::FETCH_ASSOC);
                                    $size_arr = array();
                                    foreach($result_size as $row_s) {
                                        $size_arr[] = strtolower($row_s['size_name']);
                                    }
                                    $size_str = implode(',', $size_arr);
                                    
                                    $statement_color = $pdo->prepare("SELECT t2.color_name FROM tbl_product_color t1 JOIN tbl_color t2 ON t1.color_id=t2.color_id WHERE t1.p_id=?");
                                    $statement_color->execute(array($p_id));
                                    $result_color = $statement_color->fetchAll(PDO::FETCH_ASSOC);
                                    $color_arr = array();
                                    foreach($result_color as $row_c) {
                                        $color_arr[] = strtolower($row_c['color_name']);
                                    }
                                    $color_str = implode(',', $color_arr);
                                    ?>
                                    <div class="col-md-4 item item-product-cat" data-sizes="<?php echo $size_str; ?>" data-colors="<?php echo $color_str; ?>">
                                        <div class="inner">
                                            <div class="thumb">
